...
test.describe("authoritative standard-student presentation",()=>{
  ...
  test("Premium progress and documents retain the logged-in shell while locked",async({page})=>{
    await page.goto("/dashboard");await expect(page).toHaveURL(/\/student\/dashboard$/);await expect(page.locator('[data-legacy-page="student-dashboard"]')).toHaveAttribute("data-student-state","authenticated_standard");await expect(page.locator(".avatar-heading-right-box")).toContainText(/Yet to\s+Unlock\s+Full\s+Access/i);
...
test.describe("authoritative Premium presentation",()=>{
  ...
  test("Premium retained pages keep the complete header and sidebar",async({page},testInfo)=>{
    ...
    await page.goto("/dashboard");await expect(page).toHaveURL(/\/student\/dashboard$/);await expect(page.locator("#where-you-stand")).toBeVisible();await expect(page.locator(".premium-kanban")).toHaveCount(0);
...
test.describe("audited grant/revoke state transition",()=>{
  ...
  test("grant unlocks and revoke relocks the same student identity",async({request,browser},testInfo)=>{
    ...
    const grantedContext=await browser.newContext({storageState:studentState});const grantedPage=await grantedContext.newPage();await grantedPage.goto("/student/dashboard");await expect(grantedPage.locator('.developer-student-shell[data-student-state="authenticated_premium"]')).toBeVisible();await expect(grantedPage.locator("#where-you-stand")).toBeVisible();await expect(grantedPage.locator(".premium-kanban")).toHaveCount(0);await grantedPage.goto("/feed_track_progress");await expect(grantedPage.locator(".developer-progress-page .premium-kanban")).toBeVisible();
    ...
    await grantedPage.goto("/student/dashboard?certification=revoked",{waitUntil:"networkidle"});await expect(grantedPage.locator('[data-legacy-page="student-dashboard"]')).toBeVisible();await grantedPage.goto("/feed_track_progress?certification=revoked");await expect(grantedPage.locator('[data-legacy-page="progress-locked"]')).toBeVisible();await grantedPage.goto("/upload_your_doc?certification=revoked");await expect(grantedPage.locator('[data-legacy-page="documents-locked"]')).toBeVisible();await grantedContext.close();
    ...
    const reactivatedContext=await browser.newContext({storageState:studentState});const reactivatedPage=await reactivatedContext.newPage();await reactivatedPage.goto("/student/dashboard?certification=reactivated");await expect(reactivatedPage.locator("#where-you-stand")).toBeVisible();await expect(reactivatedPage.locator(".premium-kanban")).toHaveCount(0);await reactivatedPage.goto("/feed_track_progress");await expect(reactivatedPage.locator(".premium-kanban")).toBeVisible();await reactivatedPage.goto("/upload_your_doc");await expect(reactivatedPage.locator(".developer-documents-page")).toBeVisible();await reactivatedContext.close();
