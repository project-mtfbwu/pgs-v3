...
async function submitLegacyForm(form: HTMLFormElement, page: string) {
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({ page, form: form.id || "legacy-form", fields: formPayload(form) })
    });
...
function runLegacyFormSubmission(form: HTMLFormElement, page: string) {
  void submitLegacyForm(form, page).catch(() => {
  ...
  useEffect(() => {
    ...
    root.querySelectorAll<HTMLFormElement>("form").forEach((form) => {
      ...
      form.addEventListener("submit", (event) => {
        event.preventDefault();
        event.stopPropagation();
        runLegacyFormSubmission(form, page);
      }, options);
    ...
    root.addEventListener("click", (event) => {
      ...
      if (target.closest(".premium-unlock-link, [data-premium-purchase]")) {
        event.preventDefault();
        router.push("/login?returnTo=%2Fpurplepremiumhome%23purchase");
        return;
      }

      if (target.closest(".save-program, .save-course, .heart-icon, [data-save-id]")) {
        event.preventDefault();
        setOpen(document.querySelector("#pgsLoginPopup"), true);
        return;
      }
