...
export function NotificationList({ initialItems, readOnly = false }: { initialItems: StudentNotification[]; readOnly?: boolean }) {
  ...
  return <div className="pgs-notifications">
    <div className="pgs-notifications-heading"><h2>Notifications</h2>{items.length > 0 && !readOnly && <button onClick={clearAll}>Clear all</button>}</div>
    ...
    {items.map((item) => <article key={item.id} className={item.read_at ? "is-read" : "is-unread"}>
      ...
      {readOnly ? null : <button className="pgs-notification-delete" onClick={() => remove(item.id)} aria-label={`Delete ${item.title}`}>×</button>}
    </article>)}
