...
function id(value: unknown): string {
  if (typeof value !== "string" || !/^[0-9a-f-]{36}$/i.test(value)) throw new Error("Invalid record.");
  return value;
}
...
async function context(params: Context["params"]) {
  ...
  const actor = await requirePremiumActor(studentId);
...
export async function POST(request: Request, route: Context) {
  try {
    ...
    else if (resource === "tasks") values = { ...common, column_id: id(input.column_id), title: cleanWorkspaceText(input.title, 255), details: typeof input.details === "string" ? input.details.trim().slice(0, 6000) : "", sort_order: Number.isInteger(input.sort_order) ? input.sort_order : 0, due_at: typeof input.due_at === "string" && input.due_at ? input.due_at : null, assigned_to: typeof input.assigned_to === "string" ? id(input.assigned_to) : null, created_by: actor.user.id, updated_by: actor.user.id };
    else if (resource === "alerts") values = { ...common, alert_text: cleanWorkspaceText(input.alert_text, 1000), severity: ["info", "important", "urgent"].includes(String(input.severity)) ? input.severity : "important", sort_order: Number.isInteger(input.sort_order) ? input.sort_order : 0, created_by: actor.user.id, updated_by: actor.user.id };
    else if (resource === "reviews") values = { ...common, title: cleanWorkspaceText(input.title, 255), details: typeof input.details === "string" ? input.details.trim().slice(0, 4000) : "", status: "queued", student_visible: input.student_visible !== false, sort_order: Number.isInteger(input.sort_order) ? input.sort_order : 0, created_by: actor.user.id, updated_by: actor.user.id };
    ...
    else if (resource === "requirements") values = { ...common, document_type: cleanWorkspaceText(input.document_type, 160), requirement_kind: ["required", "additional", "requested"].includes(String(input.requirement_kind)) ? input.requirement_kind : "additional", instructions: typeof input.instructions === "string" ? input.instructions.trim().slice(0, 2000) : "", sort_order: Number.isInteger(input.sort_order) ? input.sort_order : 0, requested_by: actor.user.id };
    else if (resource === "universities") values = { ...common, university_id: Number(input.university_id), stage: ["selected","shortlisted","application_started","applied","offer_received","finalized","declined"].includes(String(input.stage)) ? input.stage : "selected", sort_order: Number.isInteger(input.sort_order) ? input.sort_order : 0, created_by: actor.user.id, updated_by: actor.user.id };
  ...
  try {
    ...
    if (resource === "profile") {
      const values = {
        ...
        universities_applied: Math.max(0, Number(input.universities_applied) || 0),
        offers_received: Math.max(0, Number(input.offers_received) || 0),
    ...
    if (resource === "comments" || resource === "notes") {
      ...
      if (Number.isInteger(input.sort_order)) values.sort_order = input.sort_order;
    ...
    const supabase = await createSupabaseServerClient();
    const { error } = await supabase.from(table).update(values).eq("id", recordId).eq("student_id", actor.studentId);
...
export async function DELETE(request: Request, route: Context) {
  try {
    ...
    const input = await readJsonObject(request);
    const supabase = await createSupabaseServerClient();
    const { error } = await supabase.from(table).delete().eq("id", id(input.id)).eq("student_id", actor.studentId);
