...
async function values(request: Request) { const input = await readJsonObject(request); const type = String(input.entity_type) as keyof typeof links;const entityId=Number(input.entity_id);const tagId=Number(input.tag_id); if (!links[type] || !Number.isSafeInteger(entityId)||entityId<=0||!Number.isSafeInteger(tagId)||tagId<=0) throw new Error("Invalid tag relationship."); return { type,entityId,tagId }; }
