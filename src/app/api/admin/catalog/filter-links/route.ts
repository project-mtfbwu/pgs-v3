...
async function values(request: Request) { const input=await readJsonObject(request); const type=String(input.entity_type) as keyof typeof links; if(!links[type]||!Number.isInteger(Number(input.entity_id))||!Number.isInteger(Number(input.option_id))) throw new Error("Invalid filter relationship."); return {type,entityId:Number(input.entity_id),optionId:Number(input.option_id)}; }
