...
export async function POST(request:Request){
  ...
  try{
    await requireStaffPermission("roles.manage");const input=await readJsonObject(request);const action=String(input.action??"assign");if(!["invite","assign","revoke"].includes(action))throw new Error("Invalid staff access action.");let userId=typeof input.user_id==="string"?input.user_id:"";
    if(action==="invite"){
      if(typeof input.email!=="string"||input.email.length>320||!/^\S+@\S+\.\S+$/.test(input.email))throw new Error("Enter a valid staff email.");
      const admin=createSupabaseAdminClient();const {data,error}=await admin.auth.admin.inviteUserByEmail(input.email.trim().toLowerCase(),{data:{invited_for:"pgs_staff"}});if(error||!data.user)throw new Error("Unable to invite the staff user. Check preview SMTP and server configuration.");userId=data.user.id;invitedUserId=userId;
    }
    if(!validUuid(userId)||!["super_admin","admin","mentor","viewer"].includes(String(input.role))||!["active","suspended","ended"].includes(String(input.status??"active")))throw new Error("Invalid staff access change.");
