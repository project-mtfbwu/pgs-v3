...
export default async function PremiumDashboardPage() {
  ...
  if (state.kind!=="authenticated_premium") return <PremiumLockedState feature="dashboard" name={state.name} email={user.email??""} avatarUrl={avatarUrl}/>;
  ...
  return <ApprovedStudentShell name={name} email={user.email ?? ""} avatarUrl={avatarUrl} stateKind={state.kind} unreadCount={state.unreadCount} active="premium" contentClassName="approved-workspace-layout">
