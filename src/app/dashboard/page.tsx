...
export default async function PremiumDashboardPage() {
  ...
  return <DeveloperStudentShell name={name} email={email} avatarUrl={avatarUrl} stateKind={state.kind} unreadCount={state.unreadCount} active="premium" preview={state.preview} contentClassName="developer-premium-dashboard">
    <section className="pt-0 mobile-student-cart about-section half-section overlap-height position-relative overflow-hidden pl-100px">
      <div className="w-729px p-0"><div className="card-box-avatar mt-2"><div className="avatar-info position-relative"><div className="avatar-img"><Image src={avatarUrl} alt="" width={68} height={83} unoptimized /><div className="avatar_name"><h5>{name}</h5><span>{email}</span></div></div><div className="title-info"><h5>#purplePremium</h5><h6>{dashboard?.pathway_label || profile.study_level || "STUDENT"} PATHWAY</h6></div></div><div className="avatar-heading-right-box"><h4>#PURPLEPREMIUM</h4></div></div></div>
      ...
      <PremiumComments comments={workspace.comments} studentId={studentSubjectId(state)} readOnly={Boolean(state.preview)} />
      </div>
    </section>
