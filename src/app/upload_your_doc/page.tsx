...
export default async function DocumentsPage() {
  const state=await resolveStudentExperience();if(state.kind==="anonymous")redirect("/login?redirect=%2Fupload_your_doc");const user=state.user;const profile=state.profile;const avatarUrl=await getOwnAvatarUrl(profile.avatar_path);
  if(state.kind!=="authenticated_premium")return <PremiumLockedState feature="documents" name={state.name} avatarUrl={avatarUrl}/>;
