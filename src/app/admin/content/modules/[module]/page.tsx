...
import { AdminCrudManager, type MediaOption } from "@/components/admin-crud-manager";
import { AdminPageHeader } from "@/components/admin-page-header";
import { getAdminEntity } from "@/lib/admin-registry";
import { can, requireStaffPermission } from "@/lib/staff-auth";
import { createSupabaseServerClient } from "@/lib/supabase/server";

export default async function ContentModulePage({ params, searchParams }: { params: Promise<{ module: string }>; searchParams: Promise<{ q?: string; state?: string }> }) {
  const context = await requireStaffPermission("content.read");
  const { module: key } = await params;
  const entity = getAdminEntity("content", key);
  if (!entity) notFound();
  ...
  if (entity.fields.some((field) => field.media)) {
  ...
  return (
    <main className="ops-page">
      <AdminPageHeader eyebrow="Structured content" title={entity.label} description={entity.description} />
      <AdminCrudManager
        entity={entity}
        rows={(data ?? []) as Array<Record<string, unknown>>}
        canManage={can(context, "content.manage")}
        canPublish={can(context, "content.publish")}
