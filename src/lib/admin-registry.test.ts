...
describe("Batch 4 allow-listed operations",()=>{
  it("covers the required relational catalog and active content modules",()=>{expect(Object.keys(catalogEntities)).toEqual(expect.arrayContaining(["universities","programs","courses","events","facilitators","tags","facets","filter_options"]));expect(catalogEntities.universities.fields.some((field)=>field.key==="location")).toBe(true);expect(catalogEntities.tags.description).toMatch(/universities/);expect(Object.keys(contentEntities)).toEqual(expect.arrayContaining(["articles","faqs","testimonials","weekly_wall","key_dates","deadlines","facts","stats","people","notices","legal","social","premium_settings"]));});
